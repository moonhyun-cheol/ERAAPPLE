...
// Default game (root baseline). Per-game backups pass their own target via createBackup so a
// backup of one game can never validate/restore into another game's IndexedDB namespace.
export const DEFAULT_TARGET = { id: 'eraTHYMKR', game: 'eraTHYMKR', db: 'era-game-eraTHYMKR-erajs-v1', code: 890016222, version: 3210 };
...
// Build a backup codec + IndexedDB session lock bound to one game's identity (db, code, version).
export function createBackup(target = DEFAULT_TARGET) {
  const GAME_DB = target.db;
  const SAVE_LOCK = GAME_DB + ':session';
  ...
  return { target, identity, GAME_DB, SAVE_LOCK, MAX_BACKUP_BYTES, withSaveLock, encodeBackup, decodeBackup, packBackup, unpackBackup };
}

// Default (root game) instance keeps the original top-level API for existing callers/tests.
const defaultBackup = createBackup();
export const GAME_DB = defaultBackup.GAME_DB;
export const SAVE_LOCK = defaultBackup.SAVE_LOCK;
export const withSaveLock = defaultBackup.withSaveLock;
export const encodeBackup = defaultBackup.encodeBackup;
export const decodeBackup = defaultBackup.decodeBackup;
export const packBackup = defaultBackup.packBackup;
export const unpackBackup = defaultBackup.unpackBackup;
