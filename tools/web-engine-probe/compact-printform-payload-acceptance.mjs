...
const baselineUrl = new URL('./results/compact-printform-payload-baseline-run.json', import.meta.url);
const compactUrl = new URL('./results/compact-printform-payload-m1-run.json', import.meta.url);
if (process.argv.includes('--run')) {
  ...
  await writeFile(baselineUrl, run('engine-compact-ir-labels-lazy-slice-paged-statements.mjs'));
  await writeFile(compactUrl, run('engine-compact-ir-labels-lazy-slice-paged-statements-printform.mjs'));
}
...
assert.deepEqual(compact.deterministic, baseline.deterministic,
  'compact PrintForm payloads must preserve replay, selected state, and complete save hashes');
...
const report = {
  ...
  limitations: ['Current-game fixed path only; alternate ERA corpora remain untested.',
    ...
    'Only PrintForm Lazy wrappers are removed; parsed Form and expression trees are unchanged.']
};
const resultUrl = new URL('./results/compact-printform-payload-acceptance.json', import.meta.url);
