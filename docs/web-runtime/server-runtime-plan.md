...
상태: **계획 초안 v5 — S1 Layer A 착수(세션 코어 이식·헤드리스 검증만 완료, WS·실기기 미착수).** 이 문서는 방향 제안이며, §13에 명시된 부분외에는 구현·배포·검증된 사실이 아니다. (v5: `server` 브랜치 분기 + S1 Layer A(`server/engine-session.mjs`·`save-store.mjs`·`config.mjs`) 실제 구현·헤드리스 검증을 §11·§13에 반영. v4: §11 위생 실행 갱신, §9 호스트 토폴로지 신설, §12 iOS wss 추가)
...
| **S2 세션 수명·재연결** | 세션 1개의 유휴 타임아웃, 끊김→재연결 시 render/waiting 재동기화 | 재연결 테스트(끊고 붙여도 화면·pending 입력 보존) | 인위적 연결 끊김 주입에도 죽은 버튼/유실 0. 미달 시 S3 금지 |
...
- **그러나 남은 것은 미착수다:** WS 전송 계층(Layer B, §3 프레이밍)·실게임 61MiB 소스 서버 로드·세션 수명/재연결(S2)·얇은 클라이언트(S3)·별도 호스트 배포(§9). Layer A는 **in-process 전송**으로만 검증했고 네트워크·실기기 통과는 아직 증명하지 않았다. S1의 "Node 프로브와 동일 출력/저장 해시" 완료 기준은 replay 대조까지 가야 충족된다.
