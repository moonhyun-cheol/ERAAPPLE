// Adversarial transport reproduction of the "shop menu tap does nothing" freeze.
//
// Prior turns established: the ENGINE advances through shop-item selection (shop-buy-probe ok:true),
// and both engine builds are byte-identical at the shop (shop-engine-diff identical:true). The
// happy-path thin-client test (thin-client-shop-test.mjs) also passed. So if the freeze is real, it
// must live in the TRANSPORT/reliability layer that the WS runtime adds over the PWA worker:
// resend-on-lost-ack, out-of-order delivery, and the `if (busy) return` input drop in
// engine-session.handle. That layer's recovery paths were NOT exercised by the happy-path test.
//
// This test drives the REAL client glue (server/public/client.mjs) + the REAL engine session
// (createEngineSession) + the REAL game source through the shop, but over an ADVERSARIAL fake
// WebSocket that, deterministically:
//   (a) delays every message a few ticks in both directions,
//   (b) DROPS the first `rendered` ack and the first `waiting-ack` for each id (forcing the
//       batch-channel / waiting-relay resend paths to run),
//   (c) occasionally REORDERS an outgoing pair (events vs waiting) at the client,
//   (d) can land a client `input` inside the server's busy window (exercising `if (busy) return`).
// Engine resend intervals (8000/4000ms) are compressed via an injected scheduler so resends happen
// within the test. The assertion is the same freeze oracle: menu buttons must not end up all
// disabled, and a click must advance the session.
...
// ---- minimal DOM (same shim as thin-client-shop-test) -------------------------------------------
let OUTPUT = null;
...
// ---- deterministic PRNG for reproducible adversarial scheduling ---------------------------------
let seed = 0x9e3779b9;
function rnd() { seed = (seed * 1664525 + 1013904223) >>> 0; return seed / 0x100000000; }

// ---- compressed scheduler: shrink the engine's 8000/4000ms resend intervals for the test --------
const compressedSchedule = (fn, ms) => setTimeout(fn, Math.max(0, Math.min(ms, 12)));
const compressedUnschedule = clearTimeout;

// ---- adversarial fake WebSocket wired to the real engine session --------------------------------
let currentWS = null;
const saveMap = new Map();
function makeStore() { return { get: k => saveMap.get(k), set: (k, v) => { saveMap.set(k, v); } }; }
let SOURCE = null;
const stats = { droppedRendered: 0, droppedWaitingAck: 0, reordered: 0, resendsSeen: 0 };
const seenBatchIds = new Set();
const seenWaitingIds = new Set();
...
class AdversarialWS {
  ...
  constructor(url) {
    ...
    this.session = createEngineSession({
      ...
      schedule: compressedSchedule, unschedule: compressedUnschedule,
      post: m => this._toClient(m)
    });
    queueMicrotask(() => {
      this.readyState = 1; this._emit('open', {});
      this._deliverRaw({ type: 'session', id: 'race-session', resumed: false });
    });
  }
  addEventListener(t, fn) { (this.listeners[t] ??= []).push(fn); }
  _emit(t, ev) { for (const fn of (this.listeners[t] || [])) fn(ev); }

  // server -> client, with jittered delay and a chance to swap with the previous queued message
  _toClient(m) {
    if (m.type === 'events') { if (seenBatchIds.has(m.id)) stats.resendsSeen++; else seenBatchIds.add(m.id); }
    if (m.type === 'waiting') { if (seenWaitingIds.has(m.id)) stats.resendsSeen++; else seenWaitingIds.add(m.id); }
    const delay = 1 + Math.floor(rnd() * 4);
    const item = { m };
    if (this._deliverQueue.length && rnd() < 0.2) {
      this._deliverQueue.splice(this._deliverQueue.length - 1, 0, item); stats.reordered++;
    } else this._deliverQueue.push(item);
    setTimeout(() => this._pump(), delay);
  }
  _pump() {
    if (!this._deliverQueue.length) return;
    const item = this._deliverQueue.shift();
    this._deliverRaw(item.m);
  }
  _deliverRaw(m) { if (this.readyState === 1 || m.type === 'session') this._emit('message', { data: JSON.stringify(m) }); }

  // client -> server, with jittered delay + first-ack drops
  send(data) {
    const m = JSON.parse(data);
    if (m.type === 'rendered' && !this._acked?.has('r' + m.id)) {
      (this._acked ??= new Set()).add('r' + m.id); stats.droppedRendered++; return;
    }
    if (m.type === 'waiting-ack' && !this._acked?.has('w' + m.id)) {
      (this._acked ??= new Set()).add('w' + m.id); stats.droppedWaitingAck++; return;
    }
    const delay = 1 + Math.floor(rnd() * 3);
    setTimeout(() => { try { this.session.handle(m); } catch {} }, delay);
  }
...
globalThis.location = { protocol: 'http:', host: '127.0.0.1:8788', search: '' };
const lsMap = new Map();
...
globalThis.WebSocket = AdversarialWS;

const tick = () => new Promise(r => setTimeout(r, 4));
async function drain() { for (let i = 0; i < 120; i++) { try { await currentWS?.session.settled(); } catch {} await tick(); } }

// ---- adaptive decider (mirrors shop-buy-probe.mjs / thin-client-shop-test) -----------------------
const NAME = 'ㅅㅎ';
...
test('adversarial transport: shop menu still recovers and advances under resend/reorder/busy-drop', async () => {
  ...
  const obs = { enteredShop: false, pickedItem: false, gaveQty: false, roundsAfterPick: 0,
    reachedShop: false, buttonFreeze: null, clickAdvanced: false, inputs: 0, clicks: [], stalls: 0 };
  const fnVisits = new Map();
  let lastLen = 0;

  for (let step = 0; step < 500; step++) {
    if (probe.ended || probe.error) break;
    if (!probe.waiting) { await drain(); if (!probe.waiting) { obs.stalls++; if (obs.stalls > 3) break; continue; } }
    obs.stalls = 0;
    ...
    const clicked = !!(targetBtn && !targetBtn.disabled);
    if (clicked) OUTPUT.dispatchEvent({ type: 'click', target: targetBtn });
    else { input.value = String(choice); form.dispatchEvent({ type: 'submit', preventDefault() {} }); }
    await drain();

    if (menuish && cur.type !== 'wait') {
      const afterId = probe.waiting ? probe.waiting.id : null;
      const advanced = probe.ended || probe.error || !probe.waiting || afterId !== beforeId;
      obs.clicks.push({ step, choice, clicked, beforeId, afterId, advanced });
  ...
  console.log(JSON.stringify({
    ...
    transport: stats, clicks: obs.clicks.slice(0, 14)
  }, null, 2));

  assert.ok(stats.droppedRendered > 0 && stats.droppedWaitingAck > 0, 'test must force ack loss (resend paths)');
  assert.ok(stats.resendsSeen > 0, 'engine must have resent at least one batch/waiting');
  assert.equal(obs.enteredShop, true, 'should reach the command menu and enter the shop (107)');
  assert.equal(obs.buttonFreeze, null, 'menu buttons must not be all-disabled / un-clickable under adversity');
  assert.equal(obs.clickAdvanced, true, 'clicking a menu button must advance the session under adversity');
});
